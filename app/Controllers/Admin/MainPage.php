...
class MainPage extends BaseController
{
    ...
    public function getBanner()
    {
        ...
        $data = $this->db->table('web_banner')
            ->select('web_banner_id,concat("' . BANNER_IMG . '",web_image) as web_image,display_order,created_on,display_order,web_title,web_desc,web_link,web_btn_name,is_active')
            ->where('is_deleted', 0);
    ...
    public function saveBanner()
    {
        ...
        switch ($data['for'] ?? '') {
            ...
            case "edit":
                $manda_arr = ['web_banner_id'];
                $allowedFields = ['web_title', 'web_image','web_title','web_desc', 'web_link','web_btn_name','display_order'];
                break;
    ...
    public function pageHomeOurMission()
    {
        if (!session()->get('user_login_id')) {
            return redirect()->to(base_url(ADMIN_NAME));
        }
    ...
    public function pageAboutOurMission()
{
    ...
    public function pageCarrer()
    {
        ...
        $page['title'] = "Carrer";
        $page['breadcrumb'] = '<div class="own-breadcrumb">Carrers <i style="font-size:14px" class="fas fa-chevron-right"></i> <span>Content</span></div>';
